...
//	echo $pregbasdes;
	...
	$mo = round((($ValAuthMO - $ToT) * 0.65),2);
	$cons = round(($cons * 0.65),2);
...
if($accion==='gestionar') {
	if($_SESSION['dest']['comision'][$k] == 1) {
		...
		if($_SESSION['dest']['sub_area'][$k] == '7') { $factorHP = 0.4; } else { $factorHP = 0.6; }
		$parades = $_SESSION['dest']['sub_mo'][$k] + $_SESSION['dest']['sub_consumibles'][$k] - $_SESSION['dest']['costcons'][$k];
		$_SESSION['dest']['monto'][$k] = ($parades * $factorHP * 0.9 * 0.2) - $_SESSION['dest']['porcen'][$k];
