...
//			echo $preg2 . '<br>';
		...
		echo '</td><td>' . date('Y-m-d', strtotime($ord['orden_fecha_recepcion'])) . '</td><td>' . constant('ORDEN_ESTATUS_'.$ord['orden_estatus']) . '</td><td>' . $usuario[$ord['orden_asesor_id']] . '</td><td>' . $fevalaut . '</td><td>' . $dedu . '</td><td>' . ($mo7 + $mochm + $consumibles + $partes) . '</td><td>' . $mochm . '</td><td>' . $mo7 . '</td><td>' . $consumibles . '</td><td>' . $paraseg . '</td><td>' . $partes . '</td><td>' . $fact_num . '</td><td>' . $fe_emi . '</td></tr>'."\n";
	}
}
echo '</table>';
?>
