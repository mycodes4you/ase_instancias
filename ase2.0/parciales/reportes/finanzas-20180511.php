...
if ($f1125030 == '1' || $_SESSION['rol02']=='1') {
	...
	if($estatusflt == '5') { // --- Cobrados ---
		...
		$preg0 .= " WHERE o.orden_fecha_de_entrega >= '" . $feini . "' AND o.orden_fecha_de_entrega <= '" . $fefin . "' ";
...
<div class="page-content">
	...
	while($ord = mysql_fetch_array($matr0)) {
		...
		$preg2 = "SELECT sub_reporte, sub_aseguradora, sub_reporte FROM " . $dbpfx . "subordenes WHERE orden_id = '" . $ord['orden_id'] . "' AND sub_estatus < '190'";
				...
				while($fact = mysql_fetch_array($matr1)) {
					if($fact['fact_tipo'] < '3') {
						...
						if($hay_ajuste == 1){ // --- si hay ajuste se procesa ---
							...
							$aju_sin_iva = $ajuste['monto'] / $imp_reducir;
						} else{
							$aju_sin_iva = 0;
						}
