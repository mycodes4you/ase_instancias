...
//	echo $preg0;
	...
	$encabezado = ' OTs con ' . $nomrep . ' del ' . $t_ini . ' al ' . $t_fin;
...
<div class="page-content">
	...
	while($ord = mysql_fetch_array($matr0)) {
		$preg2 = "SELECT sub_reporte FROM " . $dbpfx . "subordenes WHERE orden_id = '" . $ord['orden_id'] . "' AND sub_aseguradora > '0' AND sub_aseguradora != '" . $ConvenioGarantia . "' AND sub_estatus < '190' GROUP BY sub_reporte";
		$matr2 = mysql_query($preg2);
		while($gsub = mysql_fetch_array($matr2)) {
			$preg3 = "SELECT sub_orden_id, sub_deducible, sub_paga_deducible FROM " . $dbpfx . "subordenes WHERE sub_reporte = '" . $gsub['sub_reporte'] . "' AND orden_id = '" . $ord['orden_id'] . "' AND sub_estatus < '190'";
//			echo $preg1;
			...
			while($sub = mysql_fetch_array($matr3)) {
				...
				if($sub['sub_paga_deducible'] > $paga_dedu) { $paga_dedu = $sub['sub_paga_deducible']; }
			}
			if($paga_dedu < 2) {
				if($export == 1) { // ---- Hoja de calculo ----
					// --- Celdas a grabar ----
