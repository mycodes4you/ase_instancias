...
*/
	...
	function bitacora($orden_id, $estatus, $dbpfx, $comentario = NULL, $interno = NULL, $sub_orden_id = NULL, $previa_id = NULL, $parausr = NULL, $etapa_com = NULL, $recordatorio = NULL) {
		global $qv_activo;
		if(basename($_SERVER['PHP_SELF'])=='recibe.php') { $deusuario = '700'; } else { $deusuario = $_SESSION['usuario'];}
		if($previa_id != '') {
			...
			$query .= "('" . $orden_id . "','";
			if($qv_activo == 1 && $interno == 'qv') { $query .= "708"; }
			else { $query .= $deusuario; }
			$query .= "','" . $estatus . "')";
			$result = mysql_query($query) or die($query);
			$bit_id = mysql_insert_id();
			$archivo = '../logs/' . date('Ymd-i') . '-base.ase';
...
// ------ La función minis tambien reduce el tamaño de la imagen principal
		...
		if ( strtolower($info['extension']) == 'jpg' || strtolower($info['type']) == 'image/jpeg') {
...
// ------ Determinar el estatus del pedido
		...
		$sql_data = array();
			$recibido = 1;
			$subtotal = 0; $iva = 0; $calc_utilidad = 'si'; $total_venta = 0; $partidas = 0;
...
// ------ Determina los pagos al pedido ------- 
			$preg_pagos = "SELECT pago_id, fact_id, monto FROM " . $dbpfx . "pagos_facturas WHERE pedido_id = '" . $pedido_id . "'";
			...
			if($recibido == 0) {
				...
				if($pedat['pedido_estatus'] < '50') {
					...
					} elseif($monto_pedido > $monto_facturas && $monto_facturas > '0' && $pagado == '0' && $pedat['pedido_estatus'] != '55') {
						$coment = 'Registro parcial de Documentos de pago sin pagos registrados';
						$sql_data['pedido_estatus'] = '15';
					} elseif($monto_pedido == $monto_facturas && $pagado == '0' && $pedat['pedido_estatus'] != '55') {
						$coment = 'Documentos de cobro registrados sin pagos registrados';
						$sql_data['pedido_estatus'] = '20';
					} elseif($pagado < $monto_pedido && $pedat['pedido_estatus'] != '55') {
						$coment = 'Pedido pagado parcialmente, Items recibidos';
						$sql_data['pedido_estatus'] = '25';
					} elseif($pagado >= $monto_pedido && $monto_pedido > $monto_facturas && $pedat['pedido_estatus'] != '55') {
...
// ------ Si el pedido está pagado, se debe marcar como pagado aún cuando no tenga facturas ---
		if($pagado >= $monto_pedido && $monto_pedido > 0) {
			if(strtotime($pedat['pedido_fecha_de_pago']) < 1000000) {
				$sql_data['pedido_fecha_de_pago'] = date('Y-m-d 00:00:01', time());
			}
