...
if($accion==='valuar') {
	...
	if($sub_orden['sub_area'] <='6' || $sub_orden['sub_area'] >='8') {
		...
		if($sub_orden['sub_siniestro']==='1') {
//			echo 'Copia y Pega las <span style="color:#f00; font-weight:bold;">PIEZAS SUSTITUIDAS que tienen precio</span>:';
...
*/
	...
	$preg0 = "SELECT op_id, op_item_seg, op_codigo, op_nombre, op_cantidad, op_precio, op_tangible, prod_id, op_pedido FROM " . $dbpfx . "orden_productos WHERE sub_orden_id = '" . $sub_orden['sub_orden_id'] . "' AND op_tangible < '3' ORDER BY op_tangible, op_nombre";
	...
	while($op = mysql_fetch_array($matr0)) {
		...
		if(($op['op_tangible'] > '0' && ($op['op_pedido'] > 0 || $sub_orden['fact_id'] > 0)) || ($op['op_tangible'] == '0' && ($sub_orden['recibo_id'] > 0 || $sub_orden['fact_id'] > 0))) {
			...
			echo '<input class="valua" style="text-align:right;" type="text" name="cantp[' . $cuenta . ']" value="' . $op['op_cantidad'] . '" size="2" />';
		}
		echo '</td>
									<td>' . $op['op_nombre'] . '</td>
									<td>' . $op['op_codigo'] . '</td>
...
elseif(($accion==='avaluo') || ($accion==='mod_avaluo')) {
	...
	if(($determina_area == 1) && ($area == 1 || $area == 6)){ // --- $determina_area definida en valores ----
...
// Obtener precio de parte
		...
		$audaprod[$i][0] = preparar_entrada_bd($descripcion);
...
// ---- Procesar consumibles ----
	...
	foreach ($audacons2 as $i => $v) {
		...
		$audaconsumible[$i][0] = preparar_entrada_bd($descripcion);
...
// ---- Procesar Mano de obra ----
	...
	foreach ($audamo2 as $i => $v) {
		...
		if($precioar == '') { $precioar = '0';}
		$audaobr[$i][0] = preparar_entrada_bd($descripcion);
...
// ---- Procesar Pintura ----
	...
	foreach ($audapi as $i => $v) {
		...
		if($precioar == '') { $precioar = '0';}
		$audap[$i][0] = preparar_entrada_bd($descripcion);
...
// ------ Si QV está activo, genera el encabezado del XML para agregar cotizaciones
					...
					if($determina_area == 1  && ($area == 1 || $area == 6)){ // --- si la variable no ha sido modificada en la tabla valores y el area en curso es 
						// --- Evaluar el área ---
...
//						echo ' se iserta en la tarea actual <br>';
						...
						echo 'autosurtido ' . $autosurtido . '<br>';
...
//						'prod_id' => '7',
						...
						'op_cantidad' => $cantmo);
					if($bloqueaprecio == '0') {
						$op_subtotal= round(($cantmo * $preciout),2);
						$sub_mo = $sub_mo + $op_subtotal;
						$presupuesto = $presupuesto + $op_subtotal;
						$sql_data_array4['op_precio'] = $preciout;
						$sql_data_array4['op_subtotal'] = $op_subtotal;
...
//			echo '<br>' . $xml;
		...
		unset($sql_data_array);
		echo 'Se actualizó la valuación de la Tarea<br>';
		...
		if($determina_area == 1 && $prods_faltantes != '' && ($area == 1 || $area == 6)){ // --- si la variable no ha sido modificada en la tabla valores y el area en curso es hojalatería o mecánica
...
// ------ Captura de Mano de Obra ------
	...
	$preg0 = "SELECT op_id, op_codigo, op_nombre, op_cantidad, op_precio_pres, prod_id, op_tangible, op_pedido FROM " . $dbpfx . "orden_productos WHERE sub_orden_id = '" . $sub_orden['sub_orden_id'] . "' ORDER BY op_tangible,op_item";
	$matr0 = mysql_query($preg0) or die("ERROR: Fallo selección de requeridos!");
	...
	while($op = mysql_fetch_array($matr0)) {
		...
		if($op['op_pedido'] > 0) {
			echo number_format($op['op_precio_pres'],2);
			echo '<input type="hidden" name="preunit[' . $cuenta . ']" value="' . number_format($op['op_precio_pres'],2) . '" size="8" />';
		} else {
			echo '<input style="text-align:right;" type="text" name="preunit[' . $cuenta . ']" value="' . number_format($op['op_precio_pres'],2) . '" size="8" />';
		}
		echo '</td><td>';
		if($op['op_pedido'] > 0){
			echo 'En Pedido';
...
//			echo $j . ' ' . $v[$j] . ' -> ' . ord($v[$j]) . '<br>'; 
		...
		$audaobr[$i][0] = preparar_entrada_bd($des);
		$audaobr[$i][1] = trim($precioar);
	}
//	print_r($audaobr);
//	echo '<br><br>';
...
//					echo 'Cant: ' .$cantp[$i] . ' PU: ' . $preunit[$i] . '<br>';
					...
					$sqldata = array('op_cantidad' => $cantp[$i], 'op_precio_pres' => $preunit[$i], 'op_subtotal_pres' => $opsub);
		...
		$refacciones=0;
		$preg1 = "SELECT prod_id, op_cantidad, op_precio_pres, op_descuento, op_tangible, op_estructural, op_recibidos FROM " . $dbpfx . "orden_productos WHERE sub_orden_id = '" . $sub_orden_id . "' AND op_tangible < '3'";
//		echo $preg1;
		$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de orden_productos!");
		while($op = mysql_fetch_array($matr1)) {
			$op_subtotal = round(($op['op_cantidad'] * ($op['op_precio_pres'] - $op['op_descuento'])),2);
			if($op['op_tangible']=='1') {
				$sub_partes = $sub_partes + $op_subtotal;
			} elseif($op['op_tangible']=='2') {
				$sub_consumibles = $sub_consumibles + $op_subtotal;
...
//						$op = mysql_fetch_array($matr1);
					...
					if($prod_tangible[$i]=='1') {
						...
						$op_subtotal= round(($prod_cantidad[$i] * $prod_precio[$i]),2);
						$sub_mo = $sub_mo + $op_subtotal;
						$tiempo = $tiempo + $prod_cantidad[$i];
					}
					$presupuesto = $presupuesto + $op_subtotal;
					$sql_data_array = ['sub_orden_id' => $sub_orden_id,
						'op_area' => $area,
						'op_item' => $item,
						'prod_id' => $prod_id[$i],
						'op_cantidad' => $prod_cantidad[$i],
						...
						'op_codigo' => $prod_codigo[$i],
						'op_tangible' => $prod_tangible[$i],
						'op_precio_pres' => $prod_precio[$i],
						'op_costo' => $prod_costo[$i],
						'op_pres' => '1',
						'op_subtotal_pres' => $op_subtotal,
					];
					$nueva_id = ejecutar_db($dbpfx . 'orden_productos', $sql_data_array, 'insertar');
					$item++;
				}
...
//			echo $preg1.'<br>';
					$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de paq_prods!");
					while($prods = mysql_fetch_array($matr1)) {
						$op_subtotal= round(($paqs['pc_prod_cant'] * $prods['prod_precio']),2);
						$presupuesto = $presupuesto + $op_subtotal;
						$preg2 = "SELECT op.prod_id, op.op_cantidad, p.prod_cantidad_existente FROM " . $dbpfx . "orden_productos op, " . $dbpfx . "productos p WHERE p.prod_id = '" . $prod_id[$i] . "' AND op.prod_id = p.prod_id";
						$matr2 = mysql_query($preg1) or die("ERROR: Fallo selección de paq_prods!");
						$op = mysql_fetch_array($matr2);
						...
						$sql_data_array = array('sub_orden_id' => $tarea['sub_orden_id'],
							...
							'op_cantidad' => $paqs['pc_prod_cant'],
							'op_tangible' => $prods['prod_tangible'],
							'op_precio_pres' => $prods['prod_precio'],
							'op_autosurtido' => $autosurtido,
							'op_pres' => '1',
							'op_subtotal_pres' => $op_subtotal);
						$nueva_id = ejecutar_db($dbpfx . 'orden_productos', $sql_data_array, 'insertar');
...
//					$op_precio = round(($audaconsumible[$i][1] / $cant1),6);
					...
					$sql_data_array3 = array('sub_orden_id' => $sub_orden_id,
						...
						'op_nombre' => $audaconsumible[$i][0],
						'op_cantidad' => $cant1,
						'op_precio_pres' => $audaconsumible[$i][1],
						'op_subtotal_pres' => $op_subtotal,
						'op_autosurtido' => $autosurtido,
						'op_pres' => '1',
						'op_tangible' => '2');
...
//						'prod_id' => '6',
					...
					if($bloqueaprecio != '1') {
						...
						$sub_mo = $sub_mo + $op_subtotal;
						$presupuesto = $presupuesto + $op_subtotal;
						$sql_data_array4['op_precio_pres'] = $preciout;
						$sql_data_array4['op_subtotal_pres'] = $op_subtotal;
					}
					$nueva_id = ejecutar_db($dbpfx . 'orden_productos', $sql_data_array4, 'insertar');
					$item++;
				}
