...
// ------ Selección individual de archivos a descargar o eliminar
			...
			while ($documento = mysql_fetch_array($matriz)) {
				...
				if($_SESSION['rol02'] == '1' || $_SESSION['rol05'] == '1' || $_SESSION['rol12'] == '1' || $_SESSION['rol13'] == '1' || $cambia_clasificación == '1'){
...
*/
	...
	function dia_habil ($dias, $corte = NULL) {
		...
		if(!is_null($corte) && date('w', $t_habil) != $corte) {
			$ndia = date('w', $t_habil);
			if(date('w', $t_habil) < $corte) {
				$agregad = $corte - $ndia;
			} else {
				$agregad = (7 + $corte) - $ndia;
			}
			$t_habil = strtotime(date('Y-m-d 00:00:00', time())) + ($agregad * 86400);
		}
