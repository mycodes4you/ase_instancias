...
function addRelacionados( $tab )
{
	...
	$this->SetXY( 10, $yencab );
	$this->SetFont('Arial','B',10);
	$length = $this->GetStringWidth( $titulo );
	$this->MultiCell(170, 4, $titulo);
...
function addConletra($letra, $pos)
{
	...
	$length = $this->GetStringWidth( $letra );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, $letra);
}
