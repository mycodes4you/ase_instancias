...
elseif(($accion==='avaluo') || ($accion==='mod_avaluo')) {
	...
	if($qv_activo == 1 && ($subacc['sub_aseguradora'] < '1' || ($subacc['sub_aseguradora'] > '0' && ($saltapres == '1' || $asesaltapres[$subacc['sub_aseguradora']] == '1')))) {
		$qv_autorizadas = 1;
	}
...
//						echo ' se iserta en la tarea actual <br>';
						...
						$item++;
						if($qv_activo == 1 && $qv_autorizadas == 1) {
							$opxml .= '			<Ref op_id="' . $nueva_id . '" op_cantidad="' . $cant1 . '" op_nombre="' . $audaprod[$i][0] . '" op_codigo="' . $audaprod[$i][3] . '" op_estatus="10" />'."\n";
						}
			...
			if($disponible == 1){ // --- SI SE ENCUENTRA TAREA DISPONIBLE, AQUÍ SE AGREGAN LOS FALTANTES ---
				...
				foreach($prods_faltantes as $key => $val){
					...
					if(is_array($audaprod)){ // --- refacciones ---
						...
						if($qv_activo == 1 && $qv_autorizadas == 1) {
								$opxml .= '			<Ref op_id="' . $nueva_id . '" op_cantidad="' . $val['op_cantidad'] . '" op_nombre="' . $val['op_nombre'] . '" op_codigo="' . $audaprod[$i][3] . '" op_estatus="10" />'."\n";
						}
					} else { // --- M.O ---
						$sql_data_array3 = [
							'sub_orden_id' => $tarea_candidata['sub_orden_id'],
