...
$preg = "SELECT orden_id, orden_vehiculo_id, orden_asesor_id, orden_grua, orden_fecha_recepcion FROM " . $dbpfx . "ordenes WHERE orden_id >= '$orden' AND (orden_estatus < '30' OR orden_estatus >= 99) LIMIT 500";
$matr = mysql_query($preg) or die("ERROR: Fallo selección de OTs! " . $preg);
// echo $preg;

echo '<table cellpadding = "2" border="1">'."\n";
echo '<tr><td>OT</td><td>SINIESTRO</td><td>PLACAS</td><td>MARCA</td><td>MODELO</td><td>GRUA</td><td>FECHA DE INGRESO</td><td>ASESOR DE SERVICIO</td><td>FECHA DE VALUACION</td><td>DEDUCIBLE</td><td>MONTO DE REPARACION</td><td>MANO DE OBRA HyM</td><td>MANO DE OBRA PINTURA</td><td>MATERIALES PINTURA</td><td>REFACCIONES RED</td><td>REFACCIONES OTROS</td><td>NUM FACTURA</td><td>FECHA FACTURA</td></tr>'."\n";

while ($ord = mysql_fetch_array($matr)) {
	$preg0 = "SELECT sub_reporte FROM " . $dbpfx . "subordenes WHERE orden_id = '" . $ord['orden_id'] . "' AND sub_estatus < 130 AND sub_aseguradora > 0 GROUP BY sub_reporte";
...
//			echo $preg2 . '<br>';
		...
		echo '	<tr><td>' . $ord['orden_id'] . '</td><td>' . $gsub['sub_reporte'] . '</td><td>' . $veh['placas'] . '</td><td>' . $veh['marca'] . '</td><td>' . $veh['tipo'] . '</td><td>';
		if($ord['orden_grua'] == 1) { echo 'Sí'; }
		elseif($ord['orden_grua'] == 2) { echo 'No'; }
		else { echo '--'; }
		echo '</td><td>' . date('Y-m-d', strtotime($ord['orden_fecha_recepcion'])) . '</td><td>' . $usuario[$ord['orden_asesor_id']] . '</td><td>' . $fevalaut . '</td><td>' . $dedu . '</td><td>' . ($mo7 + $mochm + $consumibles + $partes) . '</td><td>' . $mochm . '</td><td>' . $mo7 . '</td><td>' . $consumibles . '</td><td>' . $paraseg . '</td><td>' . $partes . '</td><td>' . $fact_num . '</td><td>' . $fe_emi . '</td></tr>'."\n";
	}
}
echo '</table>';
?>
