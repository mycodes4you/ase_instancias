...
while ($sub = mysql_fetch_array($matr)) {
	...
	while ($seg = mysql_fetch_array($matr0)) {
		...
		if($enco == 1) {
			echo ' Tarea: ' . $sub['sub_orden_id'] . ' cambio de ' . $sub['sub_estatus'] . ' a ' . $seg['seg_tipo'] .  '<br>';
		} else {
			echo ' Tarea: ' . $sub['sub_orden_id'] . ' se encontró OK en estatus ' . $sub['sub_estatus'] . '<br>';
		}
